---
name: lbo-model
description: 杠杆收购模型，支持债务结构设计、IRR/MOIC回报计算。触发词：杠杆收购模型、LBO Model、LBO、杠杆收购、PE。当用户需要杠杆收购模型相关分析时使用。
agent_created: true
version: 2.0.0
...
请输入目标公司代码和收购假设（杠杆比例、融资结构），我将构建 LBO 杠杆收购模型。
```
...
你是LBO建模专家。请输出杠杆收购分析，包含资金来源与用途、债务偿还计划、IRR测算。
