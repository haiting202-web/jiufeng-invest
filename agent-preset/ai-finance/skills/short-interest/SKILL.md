---
name: short-interest
description: 空头持仓、融券余额、做空比率分析，识别空头压力。触发词：做空分析、Short Interest、做空、融券、空头、卖空。当用户需要做空分析相关分析时使用。
agent_created: true
version: 2.0.0
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您空头持仓、融券余额、做空比率分析，识别空头压力。
```
...
你是做空分析专家。请基于用户提供的股票信息进行做空分析，输出包含：
1. 当前融券余额与历史对比
2. 做空比率（融券余额/流通股）
3. 空头持仓变化趋势
4. 做空成本（融券利率）
