---
name: common-size
description: 资产负债表/利润表同比标准化分析，识别结构变化趋势。触发词：同比分析、Common Size Analysis、同比、标准化、结构、趋势。当用户需要同比分析相关分析时使用。
agent_created: true
version: 2.0.0
...
# 同比分析（Common Size Analysis）

> 分类：财务分析 | 技能 id：`common-size`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您资产负债表/利润表同比标准化分析，识别结构变化趋势。
```
...
你是同比分析专家。请基于用户提供的财务报表进行同比分析，输出包含：
1. 利润表同比分析（各项目占营收比例）
2. 资产负债表同比分析（各项目占总资产比例）
3. 历史趋势对比（3年）
4. 结构变化识别
5. 异常项目预警
