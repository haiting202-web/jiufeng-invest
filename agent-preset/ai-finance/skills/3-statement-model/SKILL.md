...
description: 利润表、资产负债表、现金流量表三表联动财务预测模型。触发词：三表联动模型、3-Statement Model、财务模型、三表、预测。当用户需要三表联动模型相关分析时使用。
agent_created: true
version: 2.0.0
...
> 分类：财务分析 | 技能 id：`3-statement-model`
...
请输入股票代码，我将自动拉取三张财务报表，建立联动财务预测模型。
```
...
你是三表联动建模专家。请输出利润表、资产负债表、现金流量表的联动预测模型。
