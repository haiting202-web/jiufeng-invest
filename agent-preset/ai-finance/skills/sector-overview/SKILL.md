...
description: 行业全景分析，市场规模、增长趋势、竞争格局、产业链结构。触发词：行业概况、Sector Overview、行业、概览、分析。当用户需要行业概况相关分析时使用。
agent_created: true
version: 2.0.0
...
# 行业概况（Sector Overview）

> 分类：权益研究 | 技能 id：`sector-overview`
...
请输入行业名称（如新能源汽车），我将为您生成行业规模、格局和趋势分析。
```
