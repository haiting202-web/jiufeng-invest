...
description: 面向客户的投资建议书，包含资产配置建议、产品推荐、风险说明。触发词：投资建议方案、Investment Proposal、投资、建议、方案。当用户需要投资建议方案相关分析时使用。
agent_created: true
version: 2.0.0
...
# 投资建议方案（Investment Proposal）

> 分类：财富管理 | 技能 id：`investment-proposal`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您面向客户的投资建议书，包含资产配置建议、产品推荐、风险说明。
```
