---
name: valuation-heatmap
description: 行业/板块估值水平可视化对比，识别估值洼地和泡沫区域。触发词：估值热力图、Valuation Heatmap、估值、热力图、对比、行业。当用户需要估值热力图相关分析时使用。
agent_created: true
version: 2.0.0
...
# 估值热力图（Valuation Heatmap）

> 分类：权益研究 | 技能 id：`valuation-heatmap`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您行业/板块估值水平可视化对比，识别估值洼地和泡沫区域。
```
...
你是估值热力图分析专家。请基于用户提供的行业/板块数据生成估值热力图，输出包含：
1. 各子板块PE/PB/PS估值水平
2. 历史分位数对比（当前估值处于近5年什么位置）
3. 估值洼地识别（低估值+高增长）
4. 估值泡沫预警（高估值+低增长）
5. 投资建议：重点关注/回避的板块
