...
description: 追踪覆盖公司即将到来的催化剂事件：财报发布、股东大会、除权除息、监管决策等。触发词：重要事件日历、Catalyst Calendar、日历、事件、催化剂。当用户需要重要事件日历相关分析时使用。
agent_created: true
version: 2.0.0
...
# 重要事件日历（Catalyst Calendar）

> 分类：权益研究 | 技能 id：`catalyst-calendar`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您追踪覆盖公司即将到来的催化剂事件：财报发布、股东大会、除权除息、监管决策等。
```
