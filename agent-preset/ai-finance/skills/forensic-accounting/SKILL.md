...
description: 财务异常指标识别与舞弊风险预警，防范财务造假。触发词：财务舞弊识别、Forensic Accounting、舞弊、异常、预警、财务。当用户需要财务舞弊识别相关分析时使用。
agent_created: true
version: 2.0.0
...
# 财务舞弊识别（Forensic Accounting）

> 分类：权益研究 | 技能 id：`forensic-accounting`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您财务异常指标识别与舞弊风险预警，防范财务造假。
```
...
你是财务舞弊识别专家。请基于用户提供的财务数据进行舞弊风险分析，输出包含：
1. 收入异常检测（收入与现金流背离、季节性异常）
2. 资产质量检测（存货异常、应收账款周转）
3. 关联交易风险识别
4. 审计意见分析
5. 舞弊风险等级（高/中/低）与预警信号
