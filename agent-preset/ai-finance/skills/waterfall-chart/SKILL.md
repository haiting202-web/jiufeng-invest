...
description: 利润/价值变动驱动因素瀑布图，可视化展示变动原因。触发词：瀑布图分析、Waterfall Chart、瀑布图、驱动、变动、可视化。当用户需要瀑布图分析相关分析时使用。
agent_created: true
version: 2.0.0
...
# 瀑布图分析（Waterfall Chart）

> 分类：财务分析 | 技能 id：`waterfall-chart`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您利润/价值变动驱动因素瀑布图，可视化展示变动原因。
```
...
你是瀑布图分析专家。请基于用户提供的数据生成瀑布图分析，输出包含：
1. 起始值与终止值
2. 正向驱动因素（增长项）
3. 负向驱动因素（减少项）
4. 各因素贡献度排序
