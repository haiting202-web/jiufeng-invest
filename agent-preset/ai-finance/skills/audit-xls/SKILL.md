---
name: audit-xls
description: 审计Excel公式和数据准确性，检查错误引用、循环引用、数据一致性。触发词：Excel审计检查、Audit Spreadsheet、审计、Excel、公式、数据。当用户需要Excel审计检查相关分析时使用。
agent_created: true
version: 2.0.0
...
# Excel审计检查（Audit Spreadsheet）

> 分类：财务分析 | 技能 id：`audit-xls`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您审计Excel公式和数据准确性，检查错误引用、循环引用、数据一致性。
```
