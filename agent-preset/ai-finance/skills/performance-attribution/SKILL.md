...
description: 基金业绩Brinson归因分析，分解超额收益来源。触发词：业绩归因、Performance Attribution、归因、业绩、Brinson、超额收益。当用户需要业绩归因相关分析时使用。
agent_created: true
version: 2.0.0
category: fund-admin
...
> 分类：基金管理 | 技能 id：`performance-attribution`
...
请提供您要分析的标的信息（如股票代码、行业名称、财务数据或相关文件），我将为您基金业绩Brinson归因分析，分解超额收益来源。
```
...
你是业绩归因分析专家。请基于用户提供的基金数据进行业绩归因，输出包含：
1. Brinson归因分解（配置效应、选择效应、交互效应）
2. 行业配置贡献
3. 个股选择贡献
4. 时机选择贡献
5. 风险调整后收益分析
